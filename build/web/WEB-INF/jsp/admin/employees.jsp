                ...
                <% Employee currentAdmin=(Employee) session.getAttribute(AuthUtil.SESSION_USER); %>
                    ...
                    <main class="container py-5 my-5" style="min-height: 70vh;">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h2 class="mb-0" style="font-weight: 700;"><i class="fas fa-user-tie"
                                    style="color:#4f46e5;"></i> Qu?n lý Nhân viên</h2>
                        </div>
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Qu?n lý Nhân viên</li>
                            </ol>
                        </nav>
                            ...
                            <div class="row g-3 align-items-center">
                                <div class="col-auto">
                                    <a href="${pageContext.request.contextPath}/admin/employees"
                                        class="btn btn-outline-secondary"><i class="fas fa-sync-alt"></i> Làm m?i</a>
                                </div>
                                <div class="col-md-4">
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="searchKeyword"
                                            placeholder="Tìm theo tên ho?c email..." value="${keyword}">
                                ...
                                <div class="col-auto">
                                    ...
                                    <button onclick="clearFilters()" class="btn btn-sm btn-outline-secondary"><i
                                            class="fas fa-times"></i> Xóa l?c</button>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <c:choose>
                                        <c:when test="${empty employeeList}">
                                            <div class="text-center p-5 text-muted">
                                                ...
                                                <h5>Không có d? li?u nhân viên</h5>
                                                <p>${not empty keyword ? 'Không tìm th?y k?t qu? phù h?p.' : 'H? th?ng
                                                    chua có nhân viên.'}</p>
                                            </div>
                                        </c:when>
                                        <c:otherwise>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <c:if
                                                                    ...
                                                                    <span class="you-tag text-success">(B?n)</span>
                                                                </c:if>
                                                            </td>
                                                            <td>${r.email}</td>
                                                            <td>
                                                 ...
                                                 (r.roleName == 'Librarian' ? 'role-librarian' : 'role-seller')}">
                                                            ...
                                                            <td>
                                                                <c:choose>
                                                                    <c:when
                                                                        test="${r.employeeId == sessionScope.user.employeeId}">
                                                                        <span class="badge bg-secondary">Không th? khóa
                                                                            chính mình</span>
                                                                    </c:when>
                                                                    <c:when test="${r.roleName == 'Admin'}">
                                                                        <span class="badge bg-secondary">B?o v?</span>
                                                                    </c:when>
                                                                    <c:when test="${r.status == 'blocked'}">
                                                                        <form
                                                                            ...
                                                                            <button type="submit"
                                                                                ...
                                                                                onclick="return confirm('M? khóa nhân viên này?')"><i
                                ...
                                <div class="card-footer bg-white border-0 py-3">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="small text-muted">Trang <strong>${currentPage}</strong> /
                                            <strong>${totalPages}</strong> | T?ng: <strong>${totalEmployees}</strong>
                                        </div>
                                        <nav>
                    ...
                    <script>
                        function applyFilters() { var k = document.getElementById('searchKeyword').value.trim(); var s = document.getElementById('filterStatus').value; var r = document.getElementById('filterRoleId').value; var p = document.getElementById('filterPageSize').value; var u = '${pageContext.request.contextPath}/admin/employees?page=1'; if (k) u += '&keyword=' + encodeURIComponent(k); if (s) u += '&status=' + s; if (r) u += '&roleId=' + r; if (p) u += '&pageSize=' + p; window.location.href = u; }
                        function clearFilters() { window.location.href = '${pageContext.request.contextPath}/admin/employees'; }
                        function goToPage(pg) { var k = document.getElementById('searchKeyword').value.trim(); var s = document.getElementById('filterStatus').value; var r = document.getElementById('filterRoleId').value; var p = document.getElementById('filterPageSize').value; var u = '${pageContext.request.contextPath}/admin/employees?page=' + pg; if (k) u += '&keyword=' + encodeURIComponent(k); if (s) u += '&status=' + s; if (r) u += '&roleId=' + r; if (p) u += '&pageSize=' + p; window.location.href = u; }
