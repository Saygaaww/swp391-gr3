...
<div class="container py-5">
    <c:if test="${empty order}">
        ...
        <a href="<%= ctx %>/customer/orders" class="btn btn-card">← Quay lại đơn hàng</a>
    </c:if>
    ...
    <c:if test="${not empty order}">
        ...
        <div class="card mb-4">
            <div class="card-body">
                <div class="row mb-2">
                    <div class="col-md-6"><strong>Ngày đặt:</strong> ${order.createdAt}</div>
                    <div class="col-md-6"><strong>Trạng thái:</strong> <span class="badge bg-secondary">${order.status}</span></div>
                </div>
                <div class="row">
                    <div class="col-md-6"><strong>Người nhận:</strong> ${order.readerName}</div>
                    <div class="col-md-6"><strong>Email:</strong> ${order.readerEmail}</div>
                </div>
            </div>
        </div>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <c:if test="${not empty ob.bookCoverUrl}">
                                            <img src="${ob.bookCoverUrl}" alt="" class="rounded me-2" style="width:48px;height:64px;object-fit:cover;">
                                        </c:if>
                                        <div>
                                            <strong>${ob.bookTitle}</strong>
                                            <c:if test="${not empty ob.authorName}"><br><span class="text-muted small">${ob.authorName}</span></c:if>
                                        </div>
                                    </div>
                                </td>
        ...
        <c:if test="${not empty payment}">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <p class="mb-1"><strong>Phương thức:</strong> ${payment.paymentMethod}</p>
                    <p class="mb-1"><strong>Trạng thái:</strong> ${payment.paymentStatus}</p>
                    <c:if test="${not empty payment.transactionCode}">
                        <p class="mb-1"><strong>Mã giao dịch:</strong> ${payment.transactionCode}</p>
                    </c:if>
                    <c:if test="${not empty payment.paidAt}">
                        <p class="mb-0"><strong>Thanh toán lúc:</strong> ${payment.paidAt}</p>
                    </c:if>
                </div>
            </div>
        </c:if>
