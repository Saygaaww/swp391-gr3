...
<div class="container py-5">
    ...
    <c:choose>
        <c:when test="${empty historyList}">
            <p class="text-muted">Chưa có lịch sử đọc. Mở sách trong My Library và đọc để lưu tiến độ.</p>
        </c:when>
        <c:otherwise>
            <div class="table-responsive">
                <table class="table">
                    <thead>
                        <tr><th>Sách</th><th>Trang đã đọc</th><th>Tiến độ</th><th>Đọc lúc</th><th></th></tr>
                    </thead>
                    <tbody>
                        <c:forEach items="${historyList}" var="h">
                            <tr>
                                <td><strong>${h.bookTitle}</strong></td>
                                <td>${h.lastReadPosition != null ? h.lastReadPosition : 0}<c:if test="${h.bookTotalPages != null}"> / ${h.bookTotalPages}</c:if></td>
                                <td><c:if test="${h.progressPercent != null}">${h.progressPercent}%</c:if></td>
                                <td>${h.lastReadAt}</td>
                                <td><a href="<%= ctx %>/customer/read?bookId=${h.bookId}" class="btn btn-outline-dark btn-sm">Tiếp tục đọc</a></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
