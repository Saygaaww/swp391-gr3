...
<head>
    ...
    <title>Quan ly Nhan vien - Admin</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .role-librarian { background: #e0f2fe; color: #0369a1; }
        .role-seller { background: #dcfce7; color: #166534; }
        .protected-badge { background: #f0f0f0; color: #888; padding: 4px 10px; border-radius: 4px; font-size: 11px; font-weight: 600; }
        .you-tag { color: #4f46e5; font-size: 11px; font-weight: 600; }
...
<body>
    <div class="header">
        <div class="header-left">
            <h1><i class="fas fa-user-tie"></i> Quan ly Nhan vien</h1>
            <nav class="header-nav">
                ...
                <a href="${pageContext.request.contextPath}/admin/employees" class="active"><i class="fas fa-user-tie"></i> Nhan vien</a>
    ...
    <div class="container">
        <div class="breadcrumb"><a href="${pageContext.request.contextPath}/home"><i class="fas fa-home"></i> Trang chu</a><span>/</span><a href="${pageContext.request.contextPath}/admin/dashboard">Dashboard</a><span>/</span><span style="color:#555;font-weight:600;">Quan ly Nhan vien</span></div>
        ...
        <div class="stats-row">
            <div class="stat-card"><div class="stat-icon s1"><i class="fas fa-user-tie"></i></div><div class="stat-info"><h3>${totalEmployees}</h3><p>Tong nhan vien</p></div></div>
        ...
        <div class="toolbar">
            <div class="toolbar-left">
                <a href="${pageContext.request.contextPath}/admin/employees" class="btn btn-outline"><i class="fas fa-sync-alt"></i> Lam moi</a>
                <a href="${pageContext.request.contextPath}/admin/employee-form" class="btn btn-green"><i class="fas fa-plus"></i> Them nhan vien</a>
            </div>
        ...
        <div class="table-card">
            <c:choose>
                <c:when test="${empty employeeList}"><div class="empty-state"><i class="fas fa-inbox"></i><h3>Khong co nhan vien nao</h3><p><c:choose><c:when test="${not empty keyword}">Khong tim thay voi tu khoa "${keyword}"</c:when><c:otherwise>He thong chua co nhan vien.</c:otherwise></c:choose></p></div></c:when>
                <c:otherwise>
                    <table><thead><tr><th>ID</th><th>Ho ten</th><th>Email</th><th>Vai tro</th><th>Trang thai</th><th>Ngay tao</th><th style="width:170px;">Thao tac</th></tr></thead>
                    <tbody>
                        <c:forEach var="emp" items="${employeeList}">
                            <tr>
                                <td><strong>#${emp.employeeId}</strong></td>
                                <td>${emp.fullName} <c:if test="${emp.employeeId == currentEmployee.employeeId}"><span class="you-tag">(Ban)</span></c:if></td>
                                <td>${emp.email}</td>
                                <td><c:choose><c:when test="${emp.roleName == 'ADMIN'}"><span class="role-badge role-admin">ADMIN</span></c:when><c:when test="${emp.roleName == 'LIBRARIAN'}"><span class="role-badge role-librarian">LIBRARIAN</span></c:when><c:when test="${emp.roleName == 'SELLER'}"><span class="role-badge role-seller">SELLER</span></c:when><c:otherwise><span class="role-badge" style="background:#f0f0f0;color:#555;">${emp.roleName}</span></c:otherwise></c:choose></td>
                                <td><c:choose><c:when test="${emp.status == 'active'}"><span class="status-badge status-active">Active</span></c:when><c:when test="${emp.status == 'blocked'}"><span class="status-badge status-blocked">Blocked</span></c:when><c:otherwise><span class="status-badge status-inactive">${emp.status}</span></c:otherwise></c:choose></td>
                                <td style="font-size:12px;color:#888;"><fmt:formatDate value="${emp.createdAt}" pattern="dd/MM/yyyy HH:mm"/></td>
                                <td><div class="actions">
                                    <a href="${pageContext.request.contextPath}/admin/employee-form?id=${emp.employeeId}" class="btn btn-amber btn-sm"><i class="fas fa-pen"></i> Sua</a>
                                    <c:choose>
                                        <c:when test="${emp.roleName == 'ADMIN'}"><span class="protected-badge"><i class="fas fa-shield-alt"></i> Bao ve</span></c:when>
                                        <c:when test="${emp.employeeId == currentEmployee.employeeId}"><span class="protected-badge"><i class="fas fa-user"></i> Ban</span></c:when>
                                        <c:otherwise><c:choose>
                                            <c:when test="${emp.status == 'blocked'}"><form action="${pageContext.request.contextPath}/admin/employees" method="post" style="display:inline;"><input type="hidden" name="action" value="unblock"><input type="hidden" name="id" value="${emp.employeeId}"><button type="submit" class="btn btn-blue btn-sm" onclick="return confirm('Mo khoa nhan vien nay?')"><i class="fas fa-unlock"></i> Mo khoa</button></form></c:when>
                                            <c:otherwise><form action="${pageContext.request.contextPath}/admin/employees" method="post" style="display:inline;"><input type="hidden" name="action" value="block"><input type="hidden" name="id" value="${emp.employeeId}"><button type="submit" class="btn btn-red btn-sm" onclick="return confirm('Khoa nhan vien nay?')"><i class="fas fa-lock"></i> Khoa</button></form></c:otherwise>
        ...
        <c:if test="${totalPages > 1}">
            <div class="pagination-bar">
                <div class="pagination-info">Trang <strong>${currentPage}</strong> / <strong>${totalPages}</strong> | Tong: <strong>${totalEmployees}</strong> nhan vien</div>
    ...
    <script>
        function applyFilters(){var k=document.getElementById('searchKeyword').value.trim();var r=document.getElementById('filterRoleId').value;var s=document.getElementById('filterStatus').value;var p=document.getElementById('filterPageSize').value;var u='${pageContext.request.contextPath}/admin/employees?page=1';if(k)u+='&keyword='+encodeURIComponent(k);if(r)u+='&roleId='+r;if(s)u+='&status='+s;if(p)u+='&pageSize='+p;window.location.href=u;}
        function clearFilters(){window.location.href='${pageContext.request.contextPath}/admin/employees';}
        function goToPage(pg){var k=document.getElementById('searchKeyword').value.trim();var r=document.getElementById('filterRoleId').value;var s=document.getElementById('filterStatus').value;var p=document.getElementById('filterPageSize').value;var u='${pageContext.request.contextPath}/admin/employees?page='+pg;if(k)u+='&keyword='+encodeURIComponent(k);if(r)u+='&roleId='+r;if(s)u+='&status='+s;if(p)u+='&pageSize='+p;window.location.href=u;}
