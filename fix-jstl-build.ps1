$files = Get-ChildItem -Path "c:\Users\tenma\Downloads\swp391-gr3-dung-merge-hao-borrow\build\web" -Recurse -Filter *.jsp
    ...
    try {
        ...
        if ($null -ne $content) {
            ...
            if ($changed) {
                ...
                Write-Host "Updated $($file.Name) in build"
            }
        }
    } catch {
...
Write-Host "Migration complete in build."
