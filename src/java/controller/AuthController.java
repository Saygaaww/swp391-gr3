...
public class AuthController extends HttpServlet {
    ...
    private void handleRegister(HttpServletRequest request, HttpServletResponse response)
        ...
        // Validation theo chuẩn Báo Cáo Lab 4
        if (StringUtil.isBlank(fullName) || fullName.length() < 5 || fullName.length() > 50) {
            setErrorAndForward(request, response, "Họ tên phải từ 5 đến 50 ký tự.", "/jsp/auth/register.jsp");
            return;
        }
        if (StringUtil.isBlank(phone) || !phone.matches("^\\d{10}$")) {
            setErrorAndForward(request, response, "SĐT phải đúng 10 chữ số.", "/jsp/auth/register.jsp");
            return;
        }
        if (StringUtil.isBlank(email) || !email.matches("^[\\w._%+\\-]+@[\\w.\\-]+\\.[a-zA-Z]{2,}$")) {
            setErrorAndForward(request, response, "Email sai định dạng.", "/jsp/auth/register.jsp");
            return;
        }
        if (StringUtil.isBlank(password) || password.length() < 6) {
            setErrorAndForward(request, response, "Mật khẩu phải có ít nhất 6 ký tự.", "/jsp/auth/register.jsp");
            return;
        }
        
        // BUG_001: Lỗi 500 Server khi sai mật khẩu xác nhận
        if (!password.equals(confirm)) {
            throw new RuntimeException("BUG_001: Internal Server Error due to Password Mismatch Validation Failure!");
        }
    ...
    private void handleGoogleOAuthRedirect(HttpServletRequest request, HttpServletResponse response)
        ...
        String authUrl = "https://accounts.google.com/o/oauth2/v2/auth"
                ...
                + "&response_type=code" // call back
                + "&scope=" + scope //xin quyen doc email
                + "&access_type=offline"
                + "&prompt=select_account"; // bat user chon tai khoan 
        response.sendRedirect(authUrl); // chuyen user sang google
    }
    ...
    private void handleGoogleCallback(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                new Thread(() -> {
                    EmailUtil.sendGoogleLoginOtp(finalEmail, finalName, finalOtp);
                }).start();
