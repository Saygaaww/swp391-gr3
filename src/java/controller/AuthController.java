...
public class AuthController extends HttpServlet {
    ...
    private void handleLogin(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Auto-upgrade Reader password nếu đang dùng format cũ
            if (!reader.getPasswordHash().contains(":")) {
                try {
    ...
    private void handleGoogleCallback(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try {
                ...
                if (reader == null) {
                    // Tạo tài khoản mới — dùng chuỗi rỗng cho password_hash (NOT NULL constraint)
                    reader = new Reader(googleName, googleEmail, "");
                ...
                // Đăng nhập trực tiếp sau khi xác thực Google
                loginSession(request, reader);
                response.sendRedirect(request.getContextPath() + "/books");
    ...
    private boolean verifyEmployeePassword(String rawPassword, String storedHash,
        ...
        // Dùng PasswordUtil.verifyPassword hỗ trợ cả 3 format:
        // salt:hash, plain SHA-256, và plain text
        if (PasswordUtil.verifyPassword(rawPassword, storedHash)) {
            // Tự động upgrade sang salt:hash nếu đang dùng format cũ
