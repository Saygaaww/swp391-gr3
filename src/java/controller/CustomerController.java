...
public class CustomerController extends HttpServlet {
    ...
    private void handleSubmitReview(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!ownershipDAO.hasOwnership(readerId, bookId)) {
            request.getSession().setAttribute("reviewError", "Bạn chưa có thể đánh giá sách bạn đã sở hữu.");
            response.sendRedirect(request.getContextPath() + "/customer/reviews");
            return;
        }
    ...
    private void handleProcessCheckout(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            model.Reader reader = readerDAO.findById(readerId);
            if (reader != null && reader.getEmail() != null) {
                util.EmailUtil.sendPurchaseSuccessEmail(reader.getEmail(), reader.getFullName(), orderId, cartTotal);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void handleVNPayReturn(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("00".equals(vnp_ResponseCode) && vnp_TxnRef != null && vnp_TxnRef.equals(pendingRef)
            ...
            try {
                dao.ReaderDAO readerDAO = new dao.ReaderDAO();
                model.Reader reader = readerDAO.findById(pendingReaderId);
                if (reader != null && reader.getEmail() != null) {
                    util.EmailUtil.sendPurchaseSuccessEmail(reader.getEmail(), reader.getFullName(), orderIdToUse, cartTotal);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void handleCreateBorrowRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (quantity < 1 || quantity > 5) {
            request.getSession().setAttribute("errorMessage", "Số lượng mượn phải từ 1 đến 5 quyển.");
        ...
        java.time.LocalDate expectedReturnDate = expectedStartDate.plusDays(5);
        try {
        ...
        if (expectedReturnDate.isAfter(expectedStartDate.plusDays(5))) {
            request.getSession().setAttribute("errorMessage",
                    "Thời gian mượn tối đa chỉ được 5 ngày.");
            response.sendRedirect(request.getContextPath() + "/customer/borrow-request?bookId=" + bookId);
            return;
        }
        ...
        int requestId = 0;
        try {
            // BUG_004: Lỗi sập hệ thống (NullPointerException) khi người dùng đang nợ sách mà cố mượn
            dal.FineDAO fineDAO = new dal.FineDAO();
            boolean hasUnpaidFines = false;
            for (model.FineView f : fineDAO.getFinesByReader(readerId)) {
                if ("unpaid".equals(f.getStatus())) {
                    hasUnpaidFines = true; break;
                }
            }
            if (hasUnpaidFines) {
                Object nullObj = null;
                nullObj.toString(); // Cố tình gây NullPointerException (BUG_004)
            }
