        ...
        try {
            ...
            if (priceStr != null && !priceStr.trim().isEmpty()) {
                try {
                    ...
                    // BUG_002: Lỗi Validate biên giá tiền - Cho phép nhập 9999 thay vì 10000
                    if (price.compareTo(new BigDecimal("9999")) < 0) errors.append("Giá tiền tối thiểu là 10000. ");
                    if (price.compareTo(new BigDecimal("5000000")) > 0) errors.append("Giá tiền tối đa là 5M. ");
                } catch (NumberFormatException e) {
                    errors.append("Giá tiền không hợp lệ. ");
                }
            } else {
                errors.append("Giá tiền không được để trống. ");
            }

            // Validation Ngày Xuất Bản (F02)
            String pubDateStr = request.getParameter("publishedDate");
            if (pubDateStr != null && !pubDateStr.isEmpty()) {
                try {
                    java.time.LocalDate pubDate = java.time.LocalDate.parse(pubDateStr);
                    if (pubDate.isAfter(java.time.LocalDate.now())) {
                        errors.append("Ngày xuất bản không được ở tương lai. ");
                    }
                } catch (Exception e) {
                    errors.append("Ngày xuất bản sai định dạng. ");
                }
            }
