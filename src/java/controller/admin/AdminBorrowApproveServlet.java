...
public class AdminBorrowApproveServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // Lay thong tin request TRUOC KHI duyet de chac chan get duoc email reader
            BorrowRequest req = borrowDAO.getRequestById(requestId);
            ...
            if ("approve".equals(action)) {
                ...
                if (success) {
                    ...
                    if (req != null && req.getReaderEmail() != null && !req.getReaderEmail().trim().isEmpty()) {
                        System.out.println("Dang gui email den: " + req.getReaderEmail());
                        try {
                            boolean mailSent = util.EmailUtil.sendBorrowApprovedEmail(req.getReaderEmail(), req.getReaderName(), requestId, endDate.toString(), note);
                            System.out.println("Trang thai gui email approved: " + mailSent);
                        } catch (Exception e) {
                            System.err.println("Loi khi gui email approved: " + e.getMessage());
                        }
                    } else {
                        System.out.println("Khong tim thay email hoac email rong cho yeu cau ID: " + requestId);
                    }
                }
                ...
                if (success) {
                    ...
                    if (req != null && req.getReaderEmail() != null && !req.getReaderEmail().trim().isEmpty()) {
                        System.out.println("Dang gui email den: " + req.getReaderEmail());
                        try {
                            boolean mailSent = util.EmailUtil.sendBorrowRejectedEmail(req.getReaderEmail(), req.getReaderName(), requestId, note);
                            System.out.println("Trang thai gui email rejected: " + mailSent);
                        } catch (Exception e) {
                            System.err.println("Loi khi gui email rejected: " + e.getMessage());
                        }
                    } else {
                        System.out.println("Khong tim thay email hoac email rong cho yeu cau ID: " + requestId);
                    }
