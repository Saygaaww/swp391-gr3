...
public class AdminReaderListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            request.getRequestDispatcher("/WEB-INF/jsp/admin/readers.jsp").forward(request, response);
                ...
                if ("block".equals(action)) {
                    ...
                    if (roleIdStr != null && !roleIdStr.isEmpty()) {
                        try {
                            ...
                            success = readerDAO.updateReaderRole(readerId, newRoleId);
                            if (success) {
                                session.setAttribute("successMessage",
                                        "Đã cập nhật vai trò cho người dùng: " + reader.getFullName());
                                System.out.println("Changed role to " + newRoleId + " for reader ID: " + readerId);
                            } else {
                                session.setAttribute("errorMessage", "Cập nhật vai trò thất bại!");
                            }
                        } catch (NumberFormatException e) {
                            session.setAttribute("errorMessage", "Role ID không hợp lệ.");
                        }
                    } else {
                        session.setAttribute("errorMessage", "Không tìm thấy thông tin vai trò mới!");
                    }
                } else {
