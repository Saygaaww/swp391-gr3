...
public class BorrowDAO extends DBContext {
    ...
    public boolean approveRequest(int requestId, int employeeId, String note, java.time.LocalDate startDate, java.time.LocalDate dueDate) {
        ...
        String markOutOfStockSql = "UPDATE Book SET status = 'out_of_stock' WHERE BookID = ? AND stock_quantity = 0 AND status = 'active'";
        ...
        try (Connection conn = getConnection()) {
            ...
            for (BorrowRequestItem item : reqItems) {
                ...
                // Mark book as out_of_stock if stock reaches 0
                try (PreparedStatement psOutOfStock = conn.prepareStatement(markOutOfStockSql)) {
                    psOutOfStock.setInt(1, item.getBookId());
                    psOutOfStock.executeUpdate();
                }
            }
            conn.commit();
            return true;
    ...
    public List<BorrowRequest> getRequestsFiltered(String keyword, String status,
        ...
        sql.append("ORDER BY br.requested_at ASC ");
    ...
    public boolean autoReturnBook(int readerId, int borrowItemId) {
        ...
        String restoreAvailableSql = "UPDATE Book SET status = 'active' WHERE BookID = ? AND stock_quantity > 0 AND status = 'out_of_stock'";
        ...
        try (Connection conn = getConnection()) {
            ...
            if (copyId > 0) {
                ...
                if (bookId > 0) {
                    ...
                    // Restore book status to 'active' if stock is back
                    try (PreparedStatement ps = conn.prepareStatement(restoreAvailableSql)) {
                        ps.setInt(1, bookId);
                        ps.executeUpdate();
                    }
    ...
    public boolean processReturn(
        ...
        // conditionStatus: "returned", "damaged", "lost", "late"
        String finalItemStatus = ("returned".equals(conditionStatus) || "late".equals(conditionStatus)) ? "returned" : conditionStatus;
        ...
        String insertFineSql = "INSERT INTO Fine (reader_id, borrow_item_id, fine_type_id, amount, reason, status, created_at, handled_by_employee_id, evidence_image_url) VALUES (?, ?, ?, ?, ?, 'unpaid', SYSUTCDATETIME(), ?, ?)";
        ...
        try (Connection conn = getConnection()) {
            ...
            int updated = 0;
            try (PreparedStatement ps = conn.prepareStatement(updateItemSql)) {
                ps.setString(1, finalItemStatus);
                ps.setInt(2, borrowItemId);
                updated = ps.executeUpdate();
            }
            if (updated == 0) {
            ...
            // 1.25. Auto-calculate and insert Overdue Fine (skip if manual 'late' is chosen)
            if (!"late".equals(conditionStatus)) {
                String getDueDateSql = "SELECT due_date FROM Borrow_Item WHERE borrow_item_id = ?";
                try (PreparedStatement ps = conn.prepareStatement(getDueDateSql)) {
                    ps.setInt(1, borrowItemId);
                    try (ResultSet rs = ps.executeQuery()) {
                        if (rs.next()) {
                            ...
                            if (dueDate != null) {
                                ...
                                if (now.isAfter(due)) {
                                    long daysOverdue = java.time.temporal.ChronoUnit.DAYS.between(due.toLocalDate(), now.toLocalDate());
                                    if (daysOverdue > 0) {
                                        long overdueAmount = daysOverdue * 5000; // 5000 VND/day
                                        int lateFineTypeId = resolveFineTypeId(conn, "late");
                                        if (lateFineTypeId <= 0) lateFineTypeId = 1; // Generic fix if 'late' not found
                                        try (PreparedStatement psFine = conn.prepareStatement(insertFineSql)) {
                                            psFine.setInt(1, readerId);
                                            psFine.setInt(2, borrowItemId);
                                            psFine.setInt(3, lateFineTypeId);
                                            psFine.setBigDecimal(4, java.math.BigDecimal.valueOf(overdueAmount));
                                            psFine.setString(5, "Trả sách trễ hạn " + daysOverdue + " ngày");
                                            if (handledByEmployeeId != null && handledByEmployeeId > 0) {
                                                psFine.setInt(6, handledByEmployeeId);
                                            } else {
                                                psFine.setNull(6, java.sql.Types.INTEGER);
                                            }
                                            if (evidenceImageUrl != null && !evidenceImageUrl.isBlank()) {
                                                psFine.setString(7, evidenceImageUrl);
                                            } else {
                                                psFine.setNull(7, java.sql.Types.NVARCHAR);
                                            }
                                            psFine.executeUpdate();
                                        }
                                        createdFineAmount += overdueAmount;
                                        if (createdFineTypeCode == null) createdFineTypeCode = "late";
                                    }
                                }
                            }
                        }
            ...
            // 1.5. Insert Fine if damaged/lost/late and fine info is valid.
            boolean mustCreateFine = "damaged".equals(conditionStatus) || "lost".equals(conditionStatus) || "late".equals(conditionStatus);
            if (mustCreateFine) {
                ...
                try (PreparedStatement ps = conn.prepareStatement(insertFineSql)) {
                    ...
                    if (evidenceImageUrl != null && !evidenceImageUrl.isBlank()) {
                        ps.setString(7, evidenceImageUrl);
                    } else {
                ...
                // Leave fine type code as the new one (damaged/lost overrides late for notification)
            ...
            // 3. Update Book_Copy
            if (copyId > 0) {
                String copyStatus = ("returned".equals(finalItemStatus)) ? "available" : finalItemStatus;
                try (PreparedStatement ps = conn.prepareStatement(updateCopySql)) {
                ...
                if ("returned".equals(finalItemStatus)) {
                    int bookId = 0;
                    try (PreparedStatement ps = conn.prepareStatement(getBookIdSql)) {
                        ps.setInt(1, copyId);
                        try (ResultSet rs = ps.executeQuery()) {
            ...
            } else if ("returned".equals(finalItemStatus)) {
