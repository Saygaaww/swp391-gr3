...
public class BookDAO {
    ...
    public List<Book> getAllBooks() {
        ...
        String sql = "SELECT b.*, a.AuthorName, c.CategoryName "
                ...
                + "WHERE b.Status IN ('active', 'out_of_stock') "
    ...
    public Book getBookById(int bookId) {
        String sql = "SELECT b.*, a.AuthorName, c.CategoryName "
                ...
                + "LEFT JOIN Author a ON b.AuthorID = a.AuthorID "
                + "LEFT JOIN Category c ON b.CategoryID = c.CategoryID "
                + "WHERE b.BookID = ? AND b.Status IN ('active', 'out_of_stock')";
    ...
    public List<Book> searchBooksAdvanced(String title, Integer authorId, Integer categoryId,
        ...
        sql.append("WHERE b.Status IN ('active', 'out_of_stock') ");
    ...
    public List<Book> getBooksByCategory(int categoryId) {
        ...
        String sql = "SELECT b.*, a.AuthorName, c.CategoryName "
                ...
                + "LEFT JOIN Author a ON b.AuthorID = a.AuthorID "
                + "LEFT JOIN Category c ON b.CategoryID = c.CategoryID "
                + "WHERE b.CategoryID = ? AND b.Status IN ('active', 'out_of_stock') "
                + "ORDER BY b.CreatedAt DESC";
    ...
    public List<Book> getBooksByAuthor(int authorId) {
        ...
        String sql = "SELECT b.*, a.AuthorName, c.CategoryName "
                ...
                + "LEFT JOIN Author a ON b.AuthorID = a.AuthorID "
                + "LEFT JOIN Category c ON b.CategoryID = c.CategoryID "
                + "WHERE b.AuthorID = ? AND b.Status IN ('active', 'out_of_stock') "
                + "ORDER BY b.CreatedAt DESC";
    ...
    public List<String> getAvailableLanguages() {
        ...
        String sql = "SELECT DISTINCT Language FROM Book WHERE Status IN ('active', 'out_of_stock') AND Language IS NOT NULL ORDER BY Language";
    ...
    public List<Integer> getPublicationYearRange() {
        ...
        String sql = "SELECT MIN(PublicationYear) as MinYear, MAX(PublicationYear) as MaxYear FROM Book "
                + "WHERE Status IN ('active', 'out_of_stock') AND PublicationYear IS NOT NULL";
    ...
    public int getTotalBookCount() {
        String sql = "SELECT COUNT(*) FROM Book WHERE Status IN ('active', 'out_of_stock')";
     ...
     * Also marks the book as 'out_of_stock' if stock reaches 0.
     */
    public boolean reduceStock(int bookId, int quantity) {
        String reduceSql = "UPDATE Book SET stock_quantity = stock_quantity - ? WHERE BookID = ? AND stock_quantity >= ?";
        String outOfStockSql = "UPDATE Book SET status = 'out_of_stock' WHERE BookID = ? AND stock_quantity = 0 AND Status IN ('active', 'out_of_stock')";
        try (Connection con = util.DBUtil.getConnection()) {
            // Reduce stock
            try (PreparedStatement ps = con.prepareStatement(reduceSql)) {
                ps.setInt(1, quantity);
                ps.setInt(2, bookId);
                ps.setInt(3, quantity);
                int rows = ps.executeUpdate();
                if (rows == 0) return false; // not enough stock
            }
            // Mark out of stock if needed
            try (PreparedStatement ps2 = con.prepareStatement(outOfStockSql)) {
                ps2.setInt(1, bookId);
                ps2.executeUpdate();
            }
            return true;
    ...
    public String getContentPathById(int bookId) {
        String sql = "SELECT ContentPath FROM Book WHERE BookID = ? AND Status IN ('active', 'out_of_stock')";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setInt(1, bookId);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
