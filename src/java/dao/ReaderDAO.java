...
public class ReaderDAO {
    ...
    public boolean updateProfile(Reader reader) {
        String sql = "UPDATE Reader SET full_name = ?, phone = ?, avatar = ? "
                + "WHERE reader_id = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setNString(1, reader.getFullName());
    ...
    private Reader mapRow(ResultSet rs) throws SQLException {
        ...
        // Đọc avatar — hỗ trợ cả tên cột 'avatar' và 'avatar_url'
        try {
            r.setAvatarUrl(rs.getString("avatar"));
        } catch (SQLException e) {
            try { r.setAvatarUrl(rs.getString("avatar_url")); } catch (SQLException ignored) {} }
