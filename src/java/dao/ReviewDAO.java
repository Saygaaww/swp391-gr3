...
public class ReviewDAO {
    ...
    public List<Review> getByReader(int readerId) {
        ...
        String sql = """
            SELECT r.*, b.title AS book_title, b.cover_url AS book_cover_url, rd.full_name AS reader_name
            FROM Review r
    ...
    /** Danh sách review theo sách (để hiển thị trên trang chi tiết sách). */
    ...
    public Review getByReaderAndBook(int readerId, int bookId) {
        String sql = "SELECT r.*, b.title AS book_title, b.cover_url AS book_cover_url FROM Review r JOIN Book b ON r.book_id = b.book_id WHERE r.reader_id = ? AND r.book_id = ?";
        try (Connection con = DBContext.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, readerId);
            ps.setInt(2, bookId);
