...
public class PasswordUtil {
    ...
    /**
     * Alias for hashPassword() – used by
     * AdminEmployeeFormServlet/AdminReaderFormServlet
     */
     ...
     * Hỗ trợ 3 format:
     * 1. "salt:hash" (salt + pepper, format chuẩn)
     * 2. Plain SHA-256 hex 64 ký tự (không salt, không pepper)
     * 3. Plain text (legacy/dev data)
     *
     * @param rawPassword mật khẩu người dùng nhập
     * @param storedHash  chuỗi hash lưu trong DB
     */
    public static boolean verifyPassword(String rawPassword, String storedHash) {
        if (rawPassword == null || storedHash == null) {
            return false;
        }
        // Format 1: salt:hash (with pepper)
        ...
        // Format 2: Plain SHA-256 (64 hex chars, no salt, no pepper)
        if (storedHash.length() == 64 && storedHash.matches("[0-9a-fA-F]+")) {
            try {
                MessageDigest digest = MessageDigest.getInstance("SHA-256");
                byte[] hashBytes = digest.digest(rawPassword.getBytes(StandardCharsets.UTF_8));
                String plainHash = bytesToHex(hashBytes);
                return constantTimeEquals(storedHash.toLowerCase(), plainHash.toLowerCase());
            } catch (Exception e) {
                return false;
            }
        }
