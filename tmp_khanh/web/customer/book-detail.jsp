    ...
    <c:if test="${not empty book}">
        <div class="row mb-4">
            <div class="col-md-4">
                <img src="${not empty book.coverUrl ? book.coverUrl : 'https://via.placeholder.com/300x400?text=No+Cover'}"
                     class="img-fluid rounded shadow-sm" alt="${book.title}" style="max-height:420px;object-fit:cover;">
            </div>
            <div class="col-md-8">
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="<%= ctx %>/customer/add-to-cart?bookId=${book.bookId}&quantity=1" class="btn btn-card">Thêm vào giỏ</a>
                    </c:otherwise>
                </c:choose>
                <a href="<%= ctx %>/customer/browse-books" class="btn btn-outline-dark ms-2">← Duyệt sách</a>
            </div>
        </div>
        ...
        <c:if test="${not empty book.description}">
            <div class="card mb-4">
                ...
                <div class="card-body"><p class="mb-0">${book.description}</p></div>
            </div>
        </c:if>

        <div class="card">
            <div class="card-header fw-bold">Đánh giá (${not empty reviews ? reviews.size() : 0})</div>
            <div class="card-body">
                <c:choose>
                                ...
                                <li class="list-group-item px-0">
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <span class="text-warning">
                                            <c:forEach begin="1" end="${r.rating != null ? r.rating : 0}">★</c:forEach>
                                            <c:forEach begin="${r.rating != null ? r.rating + 1 : 1}" end="5">☆</c:forEach>
                                        </span>
                                    </div>
                                    <c:if test="${not empty r.comment}"><p class="mb-0 mt-1">${r.comment}</p></c:if>
                ...
                <c:if test="${not empty sessionScope.user}">
                    <p class="mt-3 mb-0"><a href="<%= ctx %>/customer/reviews">Viết đánh giá</a></p>
                </c:if>
            </div>
        </div>
    </c:if>
