...
<div class="container py-5">
    ...
    <c:if test="${not empty ownedBooks}">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form action="<%= ctx %>/customer/bookmarks" method="post" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="bookId" class="form-select" required>
                            ...
                            <c:forEach items="${ownedBooks}" var="ob">
                                <option value="${ob.bookId}" ${param.addBookId != null && param.addBookId == ob.bookId ? 'selected' : ''}>${ob.bookTitle}</option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="col-md-2">
    ...
    <c:choose>
        <c:when test="${empty bookmarks}">
            <p class="text-muted">Chưa có bookmark. Chỉ thêm được với sách bạn đã sở hữu.</p>
        </c:when>
        <c:otherwise>
