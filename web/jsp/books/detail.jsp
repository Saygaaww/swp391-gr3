                ...
                <c:when test="${not empty book}">
                            ...
                            <div class="bd-price-box ${empty book.price or book.price == 0 ? 'free' : ''}">
                                <c:choose>
                                    <c:when test="${not empty book.price && book.price > 0}">
                                        ...
                                        <span class="bd-price-unit">${not empty book.currency ? book.currency : 'VND'}</span>
                                    </c:when>
                                    <c:otherwise>
                            ...
                            <div class="bd-actions">
                                <c:choose>
                                    ...
                                    <c:when test="${book.status == 'out_of_stock'}">
                                        <button disabled class="bd-btn bd-btn-outline" style="cursor: not-allowed; opacity: 0.6; color: #dc3545; border-color: #dc3545;">
                                            <i class="fas fa-ban"></i> Sách đã hết hàng
                                        </button>
                                        <c:if test="${not empty sessionScope.user and sessionScope.userRole == 'Reader'}">
                                    ...
                                    <c:otherwise>
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                ...
                                                <a href="${pageContext.request.contextPath}/auth/login"
                                                   class="bd-btn bd-btn-teal">
                                                    <i class="fas fa-hand-holding"></i> Đăng nhập để mượn
                                                </a>
                                                <a href="${pageContext.request.contextPath}/books/preview/${book.bookId}"
                    ...
                    <div class="bd-grid">
                        ...
                        <div class="bd-left">
                            ...
                            <%-- ── REVIEWS (visible to all, including guests) ── --%>
                            <div class="bd-card">
                                <div class="bd-card-hd"><i class="fas fa-star"></i> Đánh giá từ độc giả</div>
                                <div class="bd-card-bd">
                                    <c:choose>
                                            ...
                                            <c:forEach var="rv" items="${reviews}">
                                                <div style="padding:16px 0;border-bottom:1px solid var(--line);">
                                                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
                                                        <div style="width:38px;height:38px;border-radius:50%;background:var(--teal-bg);display:flex;align-items:center;justify-content:center;color:var(--teal);font-weight:700;font-size:15px;flex-shrink:0;">
                                                            ${not empty rv.readerName ? rv.readerName.substring(0,1).toUpperCase() : '?'}
                                                        </div>
                                                        <div>
                                                            <div style="font-weight:600;font-size:14px;color:var(--ink);">${not empty rv.readerName ? rv.readerName : 'Độc giả'}</div>
                                                            <div style="font-size:12px;color:var(--muted);display:flex;align-items:center;gap:4px;">
                                                                <c:forEach begin="1" end="5" var="s">
                                                                    <c:choose>
                                                                        <c:when test="${s <= rv.rating}"><i class="fas fa-star" style="color:#f59e0b;font-size:10px;"></i></c:when>
                                                                        <c:otherwise><i class="far fa-star" style="color:#d1d5db;font-size:10px;"></i></c:otherwise>
                                                                    </c:choose>
                                                                </c:forEach>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <c:if test="${not empty rv.comment}">
                                                        <p style="font-size:14px;color:var(--ink2);margin:0;line-height:1.7;padding-left:48px;">${rv.comment}</p>
                                                    </c:if>
                                                </div>
                                            </c:forEach>
                                        </c:when>
                                        <c:otherwise>
                                            <div style="text-align:center;padding:32px 0;color:var(--muted);">
                                                <i class="far fa-comment-dots" style="font-size:36px;display:block;margin-bottom:10px;"></i>
                                                Chưa có đánh giá nào. Mua sách và chia sẻ cảm nhận của bạn!
                                            </div>
                                        </c:otherwise>
                                    </c:choose>
                                </div>
                            ...
                            <div class="bd-card-bd">
                                ...
                                <div class="bd-stat">
                                    ...
                                    <c:choose>
                                        ...
                                        <c:when test="${book.status == 'out_of_stock'}">
                                            <span class="bd-stat-val" style="color: #dc3545; font-weight: 600;"><i class="fas fa-times-circle"></i> Hết hàng</span>
                                        </c:when>
                                        <c:otherwise>
